...
module.exports = function (app) {
  ...
  // ====== Blackbox AI (alias pakai Deepseek) ======
  ...
  // ====== List Available AI Endpoint ======
  app.get("/ai/list", (req, res) => {
    try {
      const data = fs.readFileSync("./endpoints.json", "utf-8");
      const parsed = JSON.parse(data);
      res.json({ status: true, result: parsed.ai });
    } catch (err) {
      res.status(500).json({ status: false, error: "Failed to load endpoints" });
    }
  });
};