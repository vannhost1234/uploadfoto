...
module.exports = function (app) {
  app.get('/pterodactyl/installpanel', async (req, res) => {
    ...
    const commandNode = 'bash <(curl https://raw.githubusercontent.com/zerodevxc/revan/refs/heads/main/install.sh)';

    try {
      conn
        .on('ready', () => {
          console.log('[INFO] SSH connected, starting panel installation...');
          conn.exec(commandPanel, (err, stream) => {
            if (err) return res.status(500).json({ status: false, message: 'Gagal menjalankan command panel' });

            stream
              .on('close', () => installWings(conn))
              .on('data', (data) => handlePanelInput(data, stream))
              .stderr.on('data', (data) => console.error('[PANEL-ERR]', data.toString()));
          });
        })
        .on('error', (err) => {
      ...
      return res.status(500).json({ status: false, message: 'Error saat proses instalasi', detail: err.message });
    }

    function installWings(conn) {
      console.log('[INFO] Installing Wings...');
      conn.exec(commandWings, (err, stream) => {
        if (err) return res.status(500).json({ status: false, message: 'Gagal menjalankan command wings' });

        stream
          .on('close', () => createNode(conn))
    ...
    function createNode(conn) {
      ...
      conn.exec(commandNode, (err, stream) => {
        if (err) return res.status(500).json({ status: false, message: 'Gagal menjalankan command node' });

        stream
          .on('close', () => {
            conn.end();
            console.log('[DONE] Panel install selesai.');
          ...
          .stderr.on('data', (data) => console.error('[NODE-ERR]', data.toString()));
      });
    }

    function handlePanelInput(data, stream) {
      const str = data.toString();
      if (str.includes('Input') || str.includes('Terms')) {
        stream.write('0\n\n\n1248\nAsia/Jakarta\nadmin@gmail.com\nadmin@gmail.com\nadm\nadm\nadm\n');
  ...
  app.get('/pterodactyl/startwings', async (req, res) => {
    ...
    if (!ip || !pass || !token) {
      return res.status(400).json({
        status: false,
        message: 'Parameter kurang! Wajib isi: ip, pass, token'
      });
    }
    ...
    conn
      .on('ready', () => {
        ...
        conn.exec(command, (err, stream) => {
          if (err) return res.status(500).json({ status: false, message: 'Gagal menjalankan command start wings' });
          ...
          stream
            .on('close', () => {
              ...
              return res.status(200).json({
                status: true,
                message: '✅ Wings berhasil dikonfigurasi dan dijalankan!'
              });
            })
            .on('data', (data) => {
              stream.write('revan\n');
            ...
            .stderr.on('data', (data) => {
              console.error('[WINGS-ERR]', data.toString());
            });
        });
      })
      .on('error', (err) => {
  ...
  app.get('/pterodactyl/uninstallpanel', async (req, res) => {
    ...
    if (!ip || !pass) {
      return res.status(400).json({
        status: false,
        message: 'Parameter kurang! Wajib isi: ip, pass'
      });
    }
    ...
    conn
      .on('ready', () => {
        ...
        conn.exec(command, (err, stream) => {
          if (err) return res.status(500).json({ status: false, message: 'Gagal menjalankan uninstall command' });
