...
module.exports = function (app) {
  const BASE_URL = "https://cliensvrr-whm.vannhosting.my.id";

  // Proxy endpoint untuk installpanel, startwings, uninstallpanel via ?action=
  app.get('/api/pterodactyl', async (req, res) => {
    const { action } = req.query;
    if (!action) return res.status(400).json({ status: false, message: "'action' wajib diisi" });

    const validActions = {
      installpanel: "/pterodactyl/installpanel",
      startwings: "/pterodactyl/startwings",
      uninstallpanel: "/pterodactyl/uninstallpanel",
    };

    if (!validActions[action]) return res.status(400).json({ status: false, message: "Action tidak dikenal" });

    try {
      // Bangun query string dari req.query supaya diteruskan ke backend cPanel
      const queryString = new URLSearchParams(req.query).toString();
      const apiRes = await fetch(`${BASE_URL}${validActions[action]}?${queryString}`);
      const data = await apiRes.json();
      return res.status(apiRes.status).json(data);
    } catch (error) {
      return res.status(500).json({ status: false, message: "Gagal menghubungkan ke API cPanel", error: error.message });
    }
  });
