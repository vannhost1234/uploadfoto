...
export default async function handler(req, res) {
  ...
  if (!action) {
    return res.status(400).json({ status: false, message: "Parameter action diperlukan" });
  }

  // Ganti dengan domain cPanel kamu yang ada API Node.js-nya
  const BASE_URL = "https://cliensvrr-whm.vannhosting.my.id/pterodactyl";

  let targetUrl = `${BASE_URL}?action=${action}`;

  // Tambah parameter sesuai action
  if (action === "installpanel") {
    if (!ip || !pass || !panel || !node || !ram) {
      return res.status(400).json({ status: false, message: "Parameter kurang untuk installpanel" });
    }
    targetUrl += `&ip=${ip}&pass=${pass}&panel=${panel}&node=${node}&ram=${ram}`;
  }

  if (action === "startwings") {
    if (!ip || !pass || !token) {
      return res.status(400).json({ status: false, message: "Parameter kurang untuk startwings" });
    }
    targetUrl += `&ip=${ip}&pass=${pass}&token=${token}`;
  }

  if (action === "uninstallpanel") {
    if (!ip || !pass) {
      return res.status(400).json({ status: false, message: "Parameter kurang untuk uninstallpanel" });
    }
    targetUrl += `&ip=${ip}&pass=${pass}`;
  }
  ...
  try {
    ...
    return res.status(500).json({ status: false, message: "Gagal menghubungi server cPanel", error: err.message });
  }
}
