...
module.exports = function (app) {
  ...
  try {
    ...
    const userCheckRes = await fetch(`${domain}/api/application/users`, { headers });
    const userList = await userCheckRes.json();
    const existingUser = userList.data.find(u => u.attributes.email === email);
    ...
    // Build environment variables dari egg
    const env = {};
    for (const variable of eggJson.attributes.relationships.variables.data) {
