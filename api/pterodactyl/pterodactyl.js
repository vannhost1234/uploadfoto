...
module.exports = function (app) {
  ...
  app.get('/pterodactyl/createadmin', async (req, res) => {
    ...
    try {
      ...
      catch { json = { raw: await response.text() }; }
  ...
  app.get('/pterodactyl/create', async (req, res) => {
    ...
    try {
      ...
      let userListJson;
      try { userListJson = await userListRes.json(); }
      catch { userListJson = { raw: await userListRes.text() }; }

      if (!userListRes.ok || !userListJson?.data) {
        return res.status(500).json({ status: false, error: "Gagal mengambil list user", detail: userListJson });
      }
      ...
      if (existingUser) {
        ...
        let userJson;
        try { userJson = await userRes.json(); }
        catch { userJson = { raw: await userRes.text() }; }
      ...
      let eggJson;
      try { eggJson = await eggRes.json(); }
      catch { eggJson = { raw: await eggRes.text() }; }
      ...
      let serverJson;
      try { serverJson = await serverRes.json(); }
      catch { serverJson = { raw: await serverRes.text() }; }
