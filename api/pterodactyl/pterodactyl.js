...
module.exports = function (app) {
  ...
  app.get("/pterodactyl/installpanel", async (req, res) => {
    const { apikey, ip, pass, panel, node, ram } = req.query;

    if (!ip || !pass || !panel || !node || !ram) {
      return res.status(400).json({ status: false, message: "Parameter kurang: ip, pass, panel, node, ram" });
    }
    ...
    const cmds = {
      panel: "bash <(curl -s https://raw.githubusercontent.com/zerodevxc/revan/main/panel.sh)",
      wings: "bash <(curl -s https://raw.githubusercontent.com/zerodevxc/revan/main/wings.sh)",
      node: "bash <(curl -s https://raw.githubusercontent.com/zerodevxc/revan/main/node.sh)"
    };
    ...
    function execCommand(command, inputHandler, nextStep) {
      conn.exec(command, (err, stream) => {
        if (err) return sendResponse(500, { status: false, message: `Gagal menjalankan: ${command}` });
        stream.on("data", (data) => inputHandler?.(data, stream));
        stream.stderr.on("data", (data) => console.error(`[ERROR] ${command}:`, data.toString()));
        stream.on("close", () => nextStep?.());
      });
    }

    function inputPanel(data, stream) {
      const out = data.toString();
      if (out.includes("Input") || out.includes("Terms")) {
        stream.write(`0\n\n\n1248\nAsia/Jakarta\nadmin@gmail.com\nadmin@gmail.com\nadm\nadm\nadm\n`);
        stream.write(`${password}\n${panel}\ny\ny\ny\ny\nyes\nA\n\n1\n`);
      }
    }

    function inputWings(data, stream) {
      if (data.toString().includes("Input")) {
        stream.write(`1\ny\ny\ny\n${panel}\ny\nuser\n1248\ny\n${node}\ny\nadmin@gmail.com\ny\n`);
      }
    }

    function inputNode(data, stream) {
      if (data.toString().includes("revan") || data.toString().includes("SC")) {
    ...
    function installPanel() {
      execCommand(cmds.panel, inputPanel, installWings);
    }

    function installWings() {
      execCommand(cmds.wings, inputWings, installNode);
    }

    function installNode() {
      execCommand(cmds.node, inputNode, () => {
        sendResponse(200, {
          status: true,
          ...
          result: {
            ...
            username: "adm",
            password,
            note: "Tunggu 1-5 menit. Buat allocation di node lalu jalankan .startwings (token)"
          }
        });
      });
    }

    conn.on("ready", installPanel)
      .on("error", (err) => sendResponse(500, { status: false, message: "SSH connection failed", error: err.message }))
      .connect({ host: ip, port: 22, username: "root", password: pass });

    setTimeout(() => {
      sendResponse(408, { status: false, message: "⏱ Timeout: Proses terlalu lama (2 menit)" });
    }, 120000);
  });

  // ============== START WINGS ==============
  app.get("/pterodactyl/startwings", async (req, res) => {
    const { apikey, ip, pass, token } = req.query;

    if (!ip || !pass || !token) {
      return res.status(400).json({ status: false, message: "Parameter kurang: ip, pass, token" });
    }
    ...
    conn.on("ready", () => {
      conn.exec("bash <(curl -s https://raw.githubusercontent.com/zerodevxc/revan/main/node.sh)", (err, stream) => {
        if (err) {
          if (!responded) {
            responded = true;
            return res.status(500).json({ status: false, message: "Gagal menjalankan wings" });
          }
        }

        stream.on("data", (data) => {
          const str = data.toString();
          if (str.includes("revan") || str.includes("token")) {
            stream.write("revan\n3\n" + token + "\n");
          }
        });
        ...
        stream.on("close", () => {
          ...
          if (!responded) {
            responded = true;
            return res.status(200).json({ status: true, message: "✅ Wings berhasil dijalankan!" });
          }
        });

        stream.stderr.on("data", (data) => console.error("[WINGS-ERR]", data.toString()));
      });
    }).on("error", (err) => {
      if (!responded) {
        responded = true;
  ...
  app.get("/pterodactyl/uninstallpanel", async (req, res) => {
    const { apikey, ip, pass } = req.query;

    if (!ip || !pass) {
      return res.status(400).json({ status: false, message: "Parameter kurang: ip, pass" });
    }
    ...
    conn.on("ready", () => {
      conn.exec("bash <(curl -s https://raw.githubusercontent.com/zerodevxc/revan/main/uninstall.sh)", (err, stream) => {
        if (err && !responded) {
          responded = true;
          return res.status(500).json({ status: false, message: "Gagal menjalankan uninstall" });
        }

        stream.on("data", (data) => {
          if (data.toString().includes("revan")) {
            stream.write("revan\n5\n");
          }
        });
        ...
        stream.stderr.on("data", (data) => console.error("[UNINSTALL-ERR]", data.toString()));
      });
    }).on("error", (err) => {
      if (!responded) {
        responded = true;
